...
# Excercise 1: Text and Data Display
# Basic text elements
...
# Excercise 2: Data Input Components
# Text Input
st.header("Section 2")  # A new section to group interactive input components
name = st.text_input("Enter your name", "Jane Doe")  # Simple text field with a default value
description = st.text_area("Discription", "Write something...")  # Multiline text box

# Numeric input
age = st.number_input("Age", min_value=0, max_value=120, value=25)  # Number picker with min and max range
score = st.slider("Score", 0, 100, 50)  # Slider to pick a number range

# Selection widgets
option = st.selectbox("Choose an option", ["A", "B", "C"])  # Dropdown menu
...
time = st.time_input("Select time")  # Drop down time picker

# Buttons and checbox
if st.button("Click me"):  # Button runs code when clicked
    st.write("Button clicked!") 

if st.checkbox("Show/Hide"):  # Checkbox to toggle
    st.write("Visible content")  # Shows if box is checked
...
with col1:  # Everything for the left column
    st.header("Column 1")
